...
/*
static uint16_t Pitch;         // number of PWM cycles per period

void PitchChange(uint16_t Change){
	Pitch = Pitch + Change;
	return;
}

uint16_t PitchGet(void){
		return Pitch;
}
*/	
...
void BSP_Buzzer_Pitch(uint16_t frequency, uint16_t volume){
	TIMER1_CTL_R &= ~TIMER_CTL_TAEN; // disable Timer1A during setup
	PWMCycles = ClockFrequency/frequency;
  TIMER1_TAILR_R = PWMCycles - 1;  // defines when output signal is set	
	TIMER1_TAMATCHR_R = (volume*PWMCycles)>>10;// defines when output signal is cleared
                                   // enable Timer1A 16-b, PWM, inverted to match comments
  TIMER1_CTL_R |= (TIMER_CTL_TAPWML|TIMER_CTL_TAEN);

}

int16_t BSP_Buzzer_Get(void){
	return ClockFrequency/PWMCycles;	
}
...
//		BSP_LCD_DrawChar(x*6, y*10, *pt, textColor, BSP_LCD_Color565(0, 232, 0), 1);
    pt++;
    x = x+1;
...
void BSP_LCD_OutUDec(uint32_t n, int16_t textColor){
  ...
  BSP_LCD_DrawString(StX,StY,Message,textColor, ST7735_BLACK);
